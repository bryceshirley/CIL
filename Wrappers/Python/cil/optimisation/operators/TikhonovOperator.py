...
def _is_standard_form_capable(struct_operator):
    ...
    geometries, and ``WaveletOperator`` pads at the boundary: ``db4`` on a 16x16
    image has a 22x22 coefficient space (``WaveletOperator.py:114-128``). Such
    an ``L`` is a *tall isometry* — :math:`L^*L = I`, so :math:`L^*` is a left
    inverse, but :math:`LL^* \neq I` and no true inverse exists. Measured on a
    16x16 image with ``bnd_cond='zero'``: :math:`\|L^*Lx - x\|/\|x\|` is
    1.1e-07 while :math:`\|LL^*z - z\|/\|z\|` is 7.0e-01.

    The standard form applies :math:`(WL)^{-1}` to a solver iterate that is a
    general element of ``Range(L)``, not one known to lie in the image of
    :math:`L`, so a left inverse is not sufficient. Requiring ``r == m`` on top
    of orthogonality is what makes :math:`L` a genuine bijection.

    In practice this admits :math:`L = I` and orthogonal wavelets with
    ``bnd_cond='periodization'``, which is the only mode that keeps the
    coefficient space the same size as the image.
    """
...
def create_tikhonov_operator(operator, solution_geometry=None,
                             ...
                             weighted=False, tmp_domain=None,
    ...
    weighted : bool, default False
        ...
        IRLS normally turns this on afterwards through
        :meth:`WeightedStructOperator.enable_weights`, because the inner solver
        is built before IRLS is attached to it.
    ...
    ==============  ==============  ==========================  ==========================
    ``regalpha``    ``form``        :math:`L`                   result
    ==============  ==============  ==========================  ==========================
    ``== 0``        any             any                         ``operator``, unwrapped
    ``> 0``         ``'auto'``      any, ``weighted=True``      :class:`BlockTikhonovOperator`
    ``> 0``         ``'auto'``      ``None`` or a bijection     :class:`TikhonovOperator`
    ``> 0``         ``'auto'``      otherwise                   :class:`BlockTikhonovOperator`
    ``> 0``         ``'block'``     any                         :class:`BlockTikhonovOperator`
    ``> 0``         ``'standard'``  ``None`` or a bijection     :class:`TikhonovOperator`
    ``> 0``         ``'standard'``  otherwise                   ``ValueError``
    ==============  ==============  ==========================  ==========================

    ``'auto'`` asks two questions, in order.

    **Is this IRLS?** ``weighted=True`` forces the block form whatever
    :math:`L` is. IRLS re-solves from the previous outer iterate, and the
    standard form cannot warm start: LSQR in standard form damps the step, so
    from :math:`x_0` it minimises :math:`\|A(x_0 + \delta) - b\|^2 +
    \alpha^2\|\delta\|^2`, which is not the regularised problem. The block form
    carries the penalty as a row of :math:`K`, so it applies to the solution.

    **Otherwise, is** :math:`L` **an orthogonal bijection?** That is
    :func:`_is_standard_form_capable`: orthogonal *and* square. Squareness is
    not a technicality — an orthogonal wavelet with boundary padding is a tall
    isometry, where :math:`L^*` is only a left inverse. This is the supported
    scope of the standard form:

    * :math:`L = I`, and orthogonal wavelets with
      ``bnd_cond='periodization'``, which is the mode that keeps the
      coefficient space the same size as the image.
    * **Padded orthogonal wavelets** (any other ``bnd_cond`` with a filter
      longer than ``haar``) have :math:`r > m` and go to the block form.
    * **Biorthogonal wavelets** (``bior*``, ``rbio*``) are supported by
      :class:`~cil.optimisation.operators.WaveletOperator` and are genuinely
      invertible, but :math:`L^{-1} \neq L^*` and CIL exposes no ``inverse()``,
      so they go to the block form too.
    * Gradient-like operators have a non-trivial null space, so no inverse
      exists at all and the block form is the only option.

    Every excluded case is *handled*, not rejected: the block form imposes the
    same penalty :math:`\alpha^2\|WLu\|^2` without ever forming an inverse.
    Only an explicit ``form='standard'`` raises.
    """
    ...
    if form == 'auto':
        if weighted:
            # IRLS re-solves from the previous outer iterate, and the standard
            # form cannot warm start: LSQR damps the step, not the solution.
            # Only the block form is correct here, whatever L is.
            form = 'block'
            log.info("form='auto' resolved to 'block': IRLS warm starts, which "
                     "the standard form does not support.")
        else:
            form = 'standard' if _is_standard_form_capable(
                struct_operator) else 'block'
            log.info("form='auto' resolved to %r.", form)
...
class WeightedStructOperator(LinearOperator):
    ...
    **Unweighted mode.** With ``weighted=False`` the weight operator is ``None``
    ...
    by an identity. This matters for more than speed: an ``IdentityOperator``
    placeholder would have to answer ``inverse()``, which no operator in CIL
    implements. Here the call is never made, so there is nothing to dispatch.
    ...
    **Two of the four maps are conditional.** :meth:`direct` and :meth:`adjoint`
    ...
    def _fill_in_place(target, values):
        ...
        silently leaves a nested child untouched. Descending one level only was
        therefore correct for a flat Range(L) -- the gradient's -- but dropped
        every leaf below the first level when :math:`L` is a stack of struct
        operators and Range(L) is a block of blocks.
        """
    ...
    def is_invertible(self):
        ...
        :math:`L` must be a genuine orthogonal bijection, not merely a tall
    ...
    def _require_invertible(self, what):
        ...
        raise ValueError(
            ...
            "to be an orthogonal bijection. {} Use form='block', which never "
...
class BlockTikhonovOperator(BlockOperator):
    ...
    Performs analysis regularisation directly in the physical solution space
    ``u``. It represents
    ...
    never forms :math:`L^{-1}`, and the only form that warm-starts correctly:
    the penalty is a row of :math:`K`, so it applies to the solution rather than
    to the step.
    ...
    def adjoint(self, x, out=None):
        ...
        (``BlockOperator.py:314-318``), allocating a full domain container on
        every call. LSQR and CGLS call the adjoint once per iteration, so that
        is a container per iteration for the length of the solve.
...
class TikhonovOperator(LinearOperator):
    ...
    :math:`L^* W^{-1}`. A bare ``GradientOperator`` will not do, its null space
    containing the constants; neither will a biorthogonal wavelet, which is
    invertible but has :math:`L^{-1} \neq L^*`. Both take the block form.
    ...
    def enable_weights(self):
        ...
        weights = self.reg_operator.enable_weights()
        if self.tmp_domain is None:
            # WL is no longer the identity, so the staging buffer is back.
            self.tmp_domain = self.reg_operator.domain_geometry().allocate(0)
        return weights
