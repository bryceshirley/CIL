...
from cil.optimisation.operators.BlockDiagonalOperator import (
    DiagonalOperator, _BlockDiagonalOperator, _DiagonalOperator)
...
class TestDiagonalOperatorDispatch(CCPiTestClass):
    """
    Which implementation the front class picks.
    ...
    def test_plain_container_uses_the_plain_operator(self):
        operator = DiagonalOperator(rand(IG, 0))
        self.assertIsInstance(operator.operator, _DiagonalOperator)

    def test_block_container_uses_the_block_operator(self):
        diagonal, _ = flat_pair()
        operator = DiagonalOperator(diagonal)
        self.assertIsInstance(operator.operator, _BlockDiagonalOperator)
    ...
    def test_nested_container_recurses(self):
        ...
        children = operator.operator.diagonal_operator_list
        # First child is a block, so it dispatches to the block branch again;
        # second is a leaf and does not.
        self.assertIsInstance(children[0].operator, _BlockDiagonalOperator)
        self.assertIsInstance(children[1].operator, _DiagonalOperator)
...
class TestPlainDiagonalOperator(CCPiTestClass):
    """
    The non-block path is unchanged.

    These duplicate `test_Operator.TestOperator.test_DiagonalOperator` on
    purpose: that one runs against the operator exported from
    `cil.optimisation.operators`, which is a different class from this one, so
    it does not cover this file.
    """

    def test_direct_is_elementwise(self):
        diagonal, x = rand(IG, 0), rand(IG, 1)
        operator = DiagonalOperator(diagonal)
        numpy.testing.assert_allclose(operator.direct(x).as_array(),
                                      (diagonal * x).as_array())

    def test_adjoint_is_elementwise(self):
        diagonal, x = rand(IG, 0), rand(IG, 1)
        operator = DiagonalOperator(diagonal)
        numpy.testing.assert_allclose(operator.adjoint(x).as_array(),
                                      (diagonal * x).as_array())

    def test_norm_is_the_largest_entry(self):
        diagonal = rand(IG, 0)
        operator = DiagonalOperator(diagonal)
        numpy.testing.assert_allclose(operator.norm(),
                                      numpy.abs(diagonal.as_array()).max(),
                                      rtol=1e-6)
...
class TestNestedBlockDiagonalOperator(CCPiTestClass):
    ...
    def test_geometry_is_nested_rather_than_none(self):
        # Regression. The geometry used to be read off `diagonal.geometry`,
        # which is None for a block of blocks -- BlockDataContainer.geometry
        # builds a BlockGeometry from `el.geometry.copy()` and BlockGeometry has
        # no copy(), so the AttributeError is swallowed. The operator was left
        # with no domain and no range, and everything that allocates from them,
        # dot_test included, raised AttributeError on None.
        domain = self.operator.domain_geometry()
        self.assertIsInstance(domain, BlockGeometry)
    ...
    def test_matches_applying_the_leaves_one_at_a_time(self):
        ...
        # from the plain operator, so it does not reuse the block code path.
        result = self.operator.direct(self.x)
        expected = [
            _DiagonalOperator(d).direct(v)
