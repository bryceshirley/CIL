...
def load_and_process_walnut(angle_step: int = 25, 
    ...
    """
    Loads, preprocesses, and returns the walnut dataset, its Projection Operator, and FDK reconstruction.
    ...
    # Prevent log(0) or log(negative) by clipping values to a small positive number
    binned_arr = binned_data.as_array()
    binned_arr[binned_arr <= 1e-5] = 1e-5
    binned_data.fill(binned_arr)

    # Conversion: Transmission -> Absorption
    data = TransmissionAbsorptionConverter()(binned_data)

    # Clean up any lingering NaNs/Infs just to be perfectly safe
    data_arr = data.as_array()
    data_arr[np.isnan(data_arr) | np.isinf(data_arr)] = 0.0
    data.fill(data_arr)
    ...
    # --- 3. Geometry Setup & FDK Reconstruction ---
    ...
    # 1. Get standard ImageGeometry FIRST (Do not reorder beforehand!)
    ig_standard = ag3D.get_ImageGeometry()
    
    print("Reconstructing volume using FDK...")
    # 2. CIL's FDK automatically handles backend conversions internally
    recon = FDK(data, image_geometry=ig_standard).run()

    # 3. Now safely reorder both data and recon to match Astra toolbox expectations
    data.reorder('astra')
    recon.reorder('astra')
    
    # Get final Image Geometry in Astra order for the ProjectionOperator
    ig3D = data.geometry.get_ImageGeometry()
    
    # --- 4. Create Operator ---
    print("Creating ProjectionOperator...")
    try:
        A = ProjectionOperator(ig3D, data.geometry, device="gpu")
    except Exception as e:
        raise RuntimeError(f"Failed to create Astra ProjectionOperator. \nError: {e}")

    print(f"Setup Complete. Data Shape: {data.shape}")
    
    return data, A, ig3D, recon
